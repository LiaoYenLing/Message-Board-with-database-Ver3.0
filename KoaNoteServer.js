...
var notes = [ {title:'title1', body:'body1', date:'date1', time:'time1'}, 
              {title:'title2', body:'body2', date:'date2', time:'time2'}, 
              {title:'title3', body:'body3', date:'date3', time:'time3'} ];
...
app.use(route.post("/note/:id", function *save(id) {
  ...
  notes[i] = { title:title, body:body, date:date, time:time,};
  response(this.response, 200, JSON.stringify(notes[i]));
}));
...
app.use(route.post('/new', function *newNote() {
  ...
  notes.push({title:title, body:body, date:date, time:time,});
